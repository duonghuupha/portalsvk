...
class Block_six extends Controller{
    ...
    function add(){
        $code = time(); $title = $_REQUEST['title_block_6']; $link = $_REQUEST['link_block_6'];
        $image = $this->_Convert->convert_file($_FILES['image_block_6']['name'], 'image_block_6');
        $data = array("image" => $image, "status"=> 1, "code" => $code, "create_at" => date("Y-m-d H:i:s"),
                        "title" => $title, "link" => $link);
        $temp = $this->model->addObj($data);
        if($temp){
            move_uploaded_file($_FILES['image_block_6']['tmp_name'], DIR_UPLOAD.'/images/block/'.$image);
        ...
        $this->view->render("block_six/add");
    }

    function update(){
        $id = base64_decode($_REQUEST['id']); $image_old = $_REQUEST['image_old_block_6'];
        $title = $_REQUEST['title_block_6']; $link = $_REQUEST['link_block_6'];
        $image = ($_FILES['image_block_6']['name'] != '') ? $this->_Convert->convert_file($_FILES['image_block_6']['name'], 'image_block_6') : $image_old;
        $data = array("image" => $image, "create_at" => date("Y-m-d H:i:s"), "title" => $title,
                        "link" => $link);
        $temp = $this->model->updateObj($id, $data);
        if($temp){
            if($_FILES['image_block_6']['name'] != ''){
                if(move_uploaded_file($_FILES['image_block_6']['tmp_name'], DIR_UPLOAD.'/images/block/'.$image)){
    ...
    function del(){
        ...
        $this->view->render("block_six/del");
    }
    ...
    function change(){
        ...
        $this->view->render("block_six/change");
    }
/////////////////////////////////////////////////////////////////////////////////////////////////
    function form(){
