...
class Block_two extends Controller{
    ...
    function json(){
        ...
        $this->view->render('block_two/json');
    }

    function add(){
        $code = time(); $title = $_REQUEST['title_block_2']; $link = $_REQUEST['link_block_2'];
        $image = $this->_Convert->convert_file($_FILES['image_block_2']['name'], 'image_block_2');
        $data = array("image" => $image, "status"=> 1, "code" => $code, "create_at" => date("Y-m-d H:i:s"),
                        "title" => $title, "link" => $link);
        $temp = $this->model->addObj($data);
        if($temp){
            move_uploaded_file($_FILES['image_block_2']['tmp_name'], DIR_UPLOAD.'/images/block/'.$image);
        ...
        $this->view->render("block_two/add");
    }

    function update(){
        $id = base64_decode($_REQUEST['id']); $image_old = $_REQUEST['image_old_block_2'];
        $title = $_REQUEST['title_block_2']; $link = $_REQUEST['link_block_2'];
        $image = ($_FILES['image_block_2']['name'] != '') ? $this->_Convert->convert_file($_FILES['image_block_2']['name'], 'image_block_2') : $image_old;
        $data = array("image" => $image, "create_at" => date("Y-m-d H:i:s"), "title" => $title,
                        "link" => $link);
        $temp = $this->model->updateObj($id, $data);
        if($temp){
            if($_FILES['image_block_2']['name'] != ''){
                if(move_uploaded_file($_FILES['image_block_2']['tmp_name'], DIR_UPLOAD.'/images/block/'.$image)){
    ...
    function del(){
        ...
        $this->view->render("block_two/del");
    }
    ...
    function change(){
        ...
        $this->view->render("block_two/change");
    }
/////////////////////////////////////////////////////////////////////////////////////////////////
    function form(){
